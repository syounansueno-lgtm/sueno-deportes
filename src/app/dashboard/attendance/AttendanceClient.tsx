...
export default function AttendanceClient({ userId, isAdmin }: Props) {
  const supabase = createClient()
  const [dataLoaded, setDataLoaded] = useState(false)
  const [todayCard, setTodayCard] = useState<Timecard | null>(null)
  const [monthCards, setMonthCards] = useState<Timecard[]>([])
  ...
  const [allStaffCards, setAllStaffCards] = useState<Timecard[]>([])
  ...
  const monthStart2 = format(new Date(new Date().getFullYear(), new Date().getMonth(), 1), 'yyyy-MM-dd')

  // クライアント側でデータを取得（ページ表示後にバックグラウンドで取得）
  useEffect(() => {
    async function fetchData() {
      const [
        ...
        supabase.from('timecards').select('*').eq('user_id', userId).eq('date', today).maybeSingle(),
        supabase.from('timecards').select('*').eq('user_id', userId).gte('date', monthStart2).order('date', { ascending: true }),
        supabase.from('daily_reports').select('*').eq('user_id', userId).eq('date', today).maybeSingle(),
      ])

      setTodayCard(todayCardData ?? null)
      setMonthCards(monthCardsData ?? [])
      if (todayReportData) {
        setReport(todayReportData.content ?? '')
        setWeather(todayReportData.weather ?? '')
        setParticipants(todayReportData.participants_count?.toString() ?? '')
        setReportSaved(true)
      }
  ...
  async function saveEdit() {
    ...
    if (!error && data) {
      ...
      setEditDone(true)
      setTimeout(() => { setEditTarget(null); setEditDone(false) }, 1200)
    }
    setEditSaving(false)
  }
